...
void cau3(LinkedList list) {
    ...
    while (curr) {
        if (curr->data.chuyenNganh == "KHMT" && curr->data.grade <= 5) {
            if (!found) {
                cout << "Thong tin hoc vien nganh KHMT co diem tong ket nho hon bang 5" << endl;
                found = true;
            }
            getData(curr->data);
        }
    ...
    if (!found) {
        cout << "Khong co hoc vien nao nganh KHMT va co diem tong ket nho hon bang 5" << endl; 
    }
}
...
int main() {
    ...
    popFront(list);
    cout << "Danh sach hoc vien sau khi xoa hoc vien cuoi dau tien" << endl; 
    traverse(list);
