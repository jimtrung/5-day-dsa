...
void d13cnpm(LinkedList list) {
    ...
    Node* curr = list.head;
    while (curr) {
        if (curr->data.lop == "D13CNPM") {
            if (!found) {
                cout << "Nhung sinh vien thuoc D13CNPM" << endl;
                found = true;
            }
            getData(curr->data);
        }
...
int main() {
    ...
    popBack(list);
    cout << "Danh sach sinh vien sau khi xoa sinh vien cuoi la:" << endl;
    traverse(list);
