...
void cau3(LinkedList list) {
    ...
    while (curr) {
        if (curr->data.chuyenNganh == "QTANM" && curr->data.grade <= 4) {
            if (!found) {
                cout << "Thong tin hoc vien nganh QTANM co diem tong ket nho hon bang 4" << endl;
                found = true;
            }
            getData(curr->data);
        }
    ...
    if (!found) {
        cout << "Khong co hoc vien nao nganh QTANM va co diem tong ket nho hon bang 4" << endl;
    }
}
