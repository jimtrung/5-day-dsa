...
void cau3(LinkedList list) {
    ...
    Node* curr = list.head;
    while (curr) {
        if (curr->data.bill > 10) {
            if (!found) {
                cout << "Thong tin khach hang co tong tien thanh toan lon hon 10 trieu la:" << endl; 
                found = true;
            }
            getData(curr->data);
        }
    ...
    if (!found) {
        cout << "Khong co khach hang nao co tong tien thanh toan lon hon 10 trieu" << endl;
    }
}
