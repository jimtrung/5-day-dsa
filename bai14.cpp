...
void cau3(LinkedList list) {
    ...
    bool found = false;
    while (curr) {
        if (curr->data.start == "tphcm") {
            if (!found) {
                cout << "Ve may bay co diem khoi hanh o TP HCM la:" << endl;
                found = true;
            }
            getData(curr->data);
        }
    ...
    if (!found) {
        cout << "Khong co ve may bay nao khoi hanh tu TP HCM" << endl;
    }
}
...
int main() {
    ...
    popBack(list);
    cout << "Danh sach ve may bay sau khi xoa phan tu cuoi cung la:" << endl; 
    traverse(list);
